...
class Ubuntu2004PkgInstallCheckService(PkgInstallCheckService):
    def _define_behavior(self) -> Dict[PkgCatalog, Callable[[None], bool]]:
        return {
            ...
            PkgCatalog.VUNDLE: lambda _: os.system('ls ~/.vim/bundle | grep Vundle.vim > /dev/null') == 0,
            PkgCatalog.YCM: lambda _: os.system('ls ~/.vim/bundle | grep YouCompleteMe > /dev/null') == 0,
            PkgCatalog.NERD_TREE: lambda _: os.system('ls ~/.vim/bundle | grep nerdtree > /dev/null') == 0,
            PkgCatalog.CSCOPE: lambda _: subprocess.run(['which', 'cscope'], capture_output=True).returncode == 0,
            PkgCatalog.TAG_LIST: lambda _: os.system('ls ~/.vim/bundle | grep taglist.vim > /dev/null') == 0,
            PkgCatalog.EASY_TAG: lambda _: os.system('ls ~/.vim/bundle | grep vim-easytags > /dev/null') == 0,
            PkgCatalog.VIM_AIRLINE: lambda _: os.system('ls ~/.vim/bundle | grep vim-airline > /dev/null') == 0,
            PkgCatalog.XNIXC: lambda _: os.system('ls ~/.xnixc | grep .xnixc > /dev/null') == 0
        }
